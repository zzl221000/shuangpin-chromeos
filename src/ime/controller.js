...
goog.ime.chrome.os.Controller = function () {
    ...
    this.eventHandler_.listen(this.model, goog.ime.chrome.os.EventType.COMMIT,
        this.handleCommitEvent).listen(this.model, goog.ime.chrome.os.EventType.OPENING,
        this.handleOpeningEvent).listen(this.model, goog.ime.chrome.os.EventType.CLOSING,
        this.handleClosingEvent).listen(this.model, goog.ime.chrome.os.EventType.MODELUPDATED,
...
goog.ime.chrome.os.Controller.prototype.processCharKey = function (e) {
    ...
    if (text === ';' ) {
        //非分号模式和非输入状态直接使用分号
        if(!this.model.semicolonMode() || this.model.status===goog.ime.chrome.os.Status.INIT) return this.processPuncKey(e)
    }
    if (!text) {

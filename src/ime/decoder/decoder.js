 ...
 * @param {goog.ime.chrome.os.Parser} parser The num of the expected results.
 * @return {goog.ime.offline.Response} The result of the decode process,
 * including the tokens and the candidates.
 */
goog.ime.offline.Decoder.prototype.decode = function(sourceWord, resultsNum,parser) {
  if (!this.isReady() || !sourceWord) {
    return null;
  }
  if(!parser){
    parser= {parse:c => {return {tokens:null,trans:c}}}
  }
  const parsed=parser.parse(sourceWord)
  // Finds the shortest token list.
  var tokenPath = this.tokenDecoder_.getBestTokens(parsed.trans);
  if (!tokenPath) {
    return null;
  }
    ...
    if (userCommitCandidate) {
      ...
      if (index !== 0) {
        var candidate = new goog.ime.offline.Candidate(
            parsed.tokens.length, userCommitCandidate, 0);
        candidates.splice(1, 0, candidate);
      }
    }
  }
  var originalTokenList=parsed.tokens
  // Also returns the token list.
  if(!parsed.tokens){
    originalTokenList = this.tokenDecoder_.getOriginalTokens(tokenPath);
  }
